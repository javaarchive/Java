 ...
 * By javaarchive
 * Github:
 * Warning: There is no backward compatibility for this.
 * Version: 1.3
 */
import java.util.*;
import java.io.*;
public abstract class usacotools {
	public static final int INITDPS=25;
	public static Vector<Vector<Vector>> dp=new Vector<Vector<Vector>>(INITDPS);
	...
	public static void initdp() {
		dp.add(new Vector<Vector>());//PLUS
		dp.add(new Vector<Vector>());//MINUS
	}
	public static void refresh(Vector v,int data) {
		if(v.size()<data) {
			v.setSize(data+1);
		}
	}
	public class dp{
		...
		public int add(int x,int y) {
			...
			return (int) (dp.get(0).get(x).get(y));
			}catch(Exception e) {
				try{return (int) dp.get(0).get(y).get(x);}catch(Exception ea) {
					int result=x+y;
					refresh(dp.get(0),x);
					refresh(dp.get(0).get(x),y);
					dp.get(0).get(x).set(y, result);
					return result;
				}
			}
